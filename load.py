...
def get_database_connection(config):
    ...
    return create_engine(f"""mssql+pymssql://{config['DB_USER']}:
                         {config['DB_PASSWORD']}@{config['DB_HOST']}/?charset=utf8""")
    ...
    with connection.connect() as conn:
        ...
        for reading in plant_data:
            ...
            botanist_id_query = sql.text(
                "SELECT botanist_id FROM s_gamma.botanist WHERE name = (:botanist_name);")
            botanist_id = conn.execute(botanist_id_query, {
                "botanist_name": reading[6]
            }).fetchone()[0]

            plant_id = reading[0] + 1

            query = sql.text(
                """INSERT INTO s_alpha.reading 
